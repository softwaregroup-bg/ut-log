...
function LogRotate(config) {
    ...
    this.pipe(logRotateStream({
        path: path.resolve(config.workDir, 'log', config.path || 'ut5-%Y-%m-%d.log'),  // Write logs rotated by the day
        symlink: path.resolve(config.workDir, 'log', config.symlink || 'ut5.log'),    // Maintain a symlink called ut5.log
